  ...
  if (update.callback_query) {
    ...
    if (data?.startsWith("take|")) {
      ...
      const { data: booking, error } = await supabase
        ...
        .eq("id", bookingId)
        .eq("status", "new")
        .select("phone, from_city, to_city, telegram_user_id, booking_type, passengers")
        .single();
      ...
      const driverChatId = (from.id) as number;

      const typeLabel = booking.booking_type === "cargo"
        ? "📦 Жук / Почта"
        : `🚕 Такси — ${booking.passengers ?? 1} жолаушы`;

      const notifications: Promise<unknown>[] = [
        // Группадағы хабарды өзгертиу
        ...
        tgPost("sendMessage", {
          ...
          text:
            ...
            `Жолаушы сизди күтеди. Жақсы жол! 🙏`,
        }),
      ];

      // ---- Жолаушыны хабарландырыу + цифрлы баҳа кнопкалары ----
          ...
          tgPost("sendMessage", {
            ...
            reply_markup: {
              inline_keyboard: [[
                ...
                { text: "2 ⭐", callback_data: `rate|${bookingId}|2` },
                { text: "3 ⭐", callback_data: `rate|${bookingId}|3` },
                { text: "4 ⭐", callback_data: `rate|${bookingId}|4` },
                { text: "5 ⭐", callback_data: `rate|${bookingId}|5` },
              ]],
            },
          })
        );
